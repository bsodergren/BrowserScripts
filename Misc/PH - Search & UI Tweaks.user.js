...
// @grant        GM_addStyle
// @run-at       document-end
// @downloadURL https://update.sleazyfork.org/scripts/373951/PH%20-%20Search%20%20UI%20Tweaks.user.js
// @updateURL https://update.sleazyfork.org/scripts/373951/PH%20-%20Search%20%20UI%20Tweaks.meta.js
// ==/UserScript==
...
class PHSearchAndUITweaks extends BrazenBaseSearchEnhancer {
	...
	_setupComplianceFilters() {
		this._addItemTextSanitizationFilter('Censor video names by substituting offensive phrases. Each rule in separate line with comma separated target phrases. ' + 'Requires page reload to apply. Example Rule: boyfriend=stepson,stepdad')
	...
	_setupEvents() {
		if (IS_FEED_PAGE) {
			this._onAfterInitialization.push(() => ChildObserver.create().onNodesAdded((itemsAdded) => {
				let itemsList
		...
		this._onBeforeUIBuild.push(() => {
			...
			this._performOperation(UI_REMOVE_LIVE_MODELS_SECTIONS,
				() => $('.streamateContent').each((index, element) => {
					$(element).parents('.sectionWrapper:first').remove()
				}), )
		})
