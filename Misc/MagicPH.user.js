...
// @downloadURL https://update.sleazyfork.org/scripts/492700/MagicPH.user.js
// @updateURL https://update.sleazyfork.org/scripts/492700/MagicPH.meta.js
// ==/UserScript==
(() => {
	'use strict';
	 ...
	 */
	if (
		(document instanceof Document || (document instanceof XMLDocument && document.createElement('div') instanceof HTMLDivElement)) && /^image\/|^text\/plain/.test(document.contentType || '') === false && (self.userjs instanceof Object === false || userjs.UserJS !== true)) {
		userjs = self.userjs = {
			UserJS: true
		};
	}
...
`;
	...
	const dbg = (...msg) => {
		const dt = new Date();
		console.debug('[%cMagicPH%c] %cDBG', 'color: rgb(255,153,0);', '', 'color: rgb(255, 212, 0);', `[${dt.getHours()}:${('0' + dt.getMinutes()).slice(-2)}:${('0' + dt.getSeconds()).slice(-2)}]`, ...msg);
	};
	const err = (...msg) => {
	...
	const info = (...msg) => {
		console.info('[%cMagicPH%c] %cINF', 'color: rgb(255,153,0);', '', 'color: rgb(0, 186, 124);', ...msg);
	};
	const log = (...msg) => {
		console.log('[%cMagicPH%c] %cLOG', 'color: rgb(255,153,0);', '', 'color: rgb(255, 212, 0);', ...msg);
	};
	// #endregion
	/**
	...
	function globalWin() {
		...
		return (check(typeof globalThis == 'object' && globalThis) || check(typeof window == 'object' && window) || check(typeof self == 'object' && self) || check(typeof this == 'object' && this) || (function() {
			return this;
		...
		try {
			...
			if (navigator) {
				...
				const {
					platform,
					mobile
				} = userAgentData ? Object(userAgentData) : {};
				userjs.isMobile = /Mobile|Tablet/.test(userAgent ? String(userAgent) : '') || Boolean(mobile) || /Android|Apple/.test(platform ? String(platform) : '');
			} else {
	...
	class i18nHandler {
		...
		toDate(str = '') {
			...
			return new Intl.DateTimeFormat(navigator.language).format(typeof str === 'string' ? new Date(str) : str);
		}
		/**
	...
	const isBlank = (obj) => {
		return (
			(typeof obj === 'string' && Object.is(obj.trim(), '')) || ((obj instanceof Set || obj instanceof Map) && Object.is(obj.size, 0)) || (Array.isArray(obj) && Object.is(obj.length, 0)) || (isObj(obj) && Object.is(Object.keys(obj).length, 0)));
	};
	/**
	 ...
	 */
	const observe = (element, listener, options = {
		subtree: true,
		childList: true
	}) => {
	...
	class HandlePage {
		...
		inject(callback, doc) {
			...
			try {
				...
				info({
					Site: this.webpage.origin,
					isMobile,
					validDomain: this.cache.validDomain
				});
				if (isMobile) {
			...
			return {
				...
				find() {
					...
					if (/tube8/.test(p)) {
						...
						this['controls-background'] = 'var(--overlay-color, var(--mph-controls-bg-color, hsla(0, 0%, 0%, 0.5)))';
	...
	class Timeout {
		 ...
		 */
		set = (localDelay, reason) => new Promise((resolve, reject) => {
			const id = setTimeout(() => {
	...
	const Network = {
		async req(url, method = 'GET', responseType = 'json', data, useFetch = false) {
			...
			return new Promise((resolve, reject) => {
				if (isGM && !useFetch) {
					...
					fetch(url, params).then((response_1) => {
		...
		prog(evt) {
			return Object.is(evt.total, 0) ? Network.format(evt.loaded) : `${+((evt.loaded / evt.total) * 100).toFixed(2)}%`;
		},
			...
			if (isNull(host)) {
				...
				const siteLoader = async (val) => {
					...
					if (val.startsWith('http')) {
						...
						await loadMedia(value.url);
					} else if (/onlyfans/.test(HP.current.root) && /^\/((?:[a-z0-9][a-z0-9._-]+))(?:\/(?=$))?$/i.test(val)) {
						value.url = `https://onlyfans.com${val}/videos`;
						await loadMedia(value.url);
					} else if (/onlyfans/.test(HP.current.root) && /^\/((?:[a-z0-9][a-z0-9._-]+))\/((?:media|photos|videos|audios|likes|streams|upcoming-streams))(?:\/(?=$))?$/i.test(val)) {
			...
			try {
				...
				if (cmd === 'open-tab' && dataset.webpage) {
					...
					if (!pageArea) {
						...
						if (HP.supported) {
							const shadow = pageArea.attachShadow({
								mode: 'closed'
							});
	...
	const addToWrapper = () => {
		...
		for (const [userId, userVideos] of videoCache) {
			const mphList = qs(`mph-list > mph-list[data-host="${userId}"]`) ?? make('mph-list', 'mph_of_list', {
				dataset: {
					host: userId
				}
			});
			...
			try {
				...
				return await new Promise((resolve, reject) => {
					...
					const mf = mFiles.filter((file) => {
						if (/get_media\?s=/.test(file) || /media\/mp4\?s=/.test(file) || /youporn|tube8/gi.test(file)) {
							return true;
						}
						return false;
					});
			...
			try {
				const handleDoc = async (htmlDocument) => {
					...
					for (const s of selected.getElementsByTagName('script')) {
						...
						const siteMedia = [/(https:[\\/]+\D+4\?s=)+(\w|\d)+/g.test(txt), /https:[\\/\w.]+tube8[\\/_.?=\w\d]+media[\\/_.?=\w\d]+/gi.test(txt), /https:[\\/\w.]+thumbzilla[\\/_.?=\w\d]+media[\\/_.?=\w\d&]+/gi.test(txt), /media_[\d]=+[\w\d\\*/+\s]+;/g.test(txt), /https:[\\/\w.]+pornhub[\\/_.?=\w\d]+media[\\/_.?=\w\d&]+/gi.test(txt), /mediaDefinition: (\[.*?\]),/g.test(txt)];
	...
	const getVidTitle = (pgUrl) => {
		...
		try {
			...
			if (/pornhub/.test(root)) {
				...
				const playervars = /redtube/.test(root) ? video_player_setup[Object.keys(video_player_setup)[0]].playervars : video_player_setup.playervars;
		...
		try {
			...
			if (isMobile) {
				...
				if (!injInto.contains(vidQuality)) {
					...
					dom.prop(vidQuality, 'innerHTML', `${iconSVG.mobileDownload}<div class="mgp_value">Quality(s)</div>`);
			...
			if (HP.host.includes('pornhub')) {
				...
				const makeData = async (JSON_SHORTIES) => {
					for (const s of normalizeTarget(JSON_SHORTIES)) {
						...
						const qualities = q.map((i) => i.videoUrl).filter((i) => !cache.includes(i)).sort(sortVideos);
						if (isEmpty(qualities)) {
							continue;
						}
				...
				const loadPage = async (page = 1) => {
					...
					if (page > 1) {
						const sURL = '/shorties/get?' + new URLSearchParams({
							token: requestToken,
							page
						});
	...
	const triggerHls = async (id) => {
		...
		const {
			file
		} = await Network.req(`https://store.externulls.com/facts/file/${id}`);
		const {
			hls_resources
		} = file;
			...
			if (/onlyfans/.test(current.root)) {
				...
				const filterPosts = (postArr) => {
					const media = postArr.filter((i) => {
						...
						if (normalizeTarget(i?.media).filter((m) => m.type === 'video' && (m.src || m.full)).length === 0) {
							return false;
						}
						return true;
					});
					...
					const mediaObj = media.map((i) => {
						const obj = {
							...i,
							media: normalizeTarget(i.media).filter((m) => m.type === 'video').map((m) => {
								return {
									...m
								};
							})
						...
						for (const m of normalizeTarget(p.media)) {
							...
							const createVideo = () => {
								const vidObj = {
									...
									original: p.responseType === 'post' ? `/${p.id}/${userData.username}` : p.responseType === 'message' ? `/my/chats/chat/${userData.id}/` : `/${p.id}`,
									text: p.text ?? null,
									raw: p
								};
								if (vidObj.src) {
