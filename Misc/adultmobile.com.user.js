// ==UserScript==
// @name        adultmobile.com
// @namespace   Violentmonkey Scripts
// @match       https://*.adultmobile.com/list/*
// @version     1.0.2
// @author      b
// @grant       none
// @description 2/23/2026, 5:33:22 PM
// ==/UserScript==
(function () {
	'use strict';
	var btnContainer = document.querySelector('#root');
	var linkGrabberBtn = document.createElement('button');
	linkGrabberBtn.style = 'position: absolute; color: white; background: black;'; //playlistButtons tooltipTrig js-pop
	var span = document.createElement('span');
	span.innerText = 'Get all video links';
	span.className = 'text';
	span.setAttribute('onclick', 'grabVideoList()');
	linkGrabberBtn.appendChild(span);
	btnContainer.appendChild(linkGrabberBtn);

	function grabVideoList() {
		var linkList = document.querySelectorAll('picture');
		for (var i = 0; i < linkList.length; i++) {
			console.log(linkList[i].childNodes[0].getAttribute('srcset'));
		}
		showLinks(linkList);
	}
	...
	function showLinks(linkList) {
		...
		for (var i = 0; i < linkList.length; i++) {
			...
			a.innerHTML = linkList[i].childNodes[0].getAttribute('srcset');
			a.href = linkList[i].childNodes[0].getAttribute('srcset');
